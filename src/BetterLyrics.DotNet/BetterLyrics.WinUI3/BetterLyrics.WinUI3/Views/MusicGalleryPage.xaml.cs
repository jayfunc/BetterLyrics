...
public sealed partial class MusicGalleryPage : Page
{
    ...
    private void AnimateTransform(DependencyObject transform, string property, double toValue, double durationSeconds)
    {
    ...
    private void ScatteredAlbum_PointerEntered(object sender, PointerRoutedEventArgs e)
    {
        if (sender is Grid grid && grid.DataContext is ScatteredAlbumModel model)
        {
            var container = VisualTreeHelper.GetParent(grid) as ContentPresenter;
            if (container != null)
            {
                Canvas.SetZIndex(container, 1000);
            }

            if (grid.RenderTransform is TransformGroup group && group.Children.Count >= 2)
            {
                if (group.Children[0] is RotateTransform rotate)
                    AnimateTransform(rotate, "Angle", 0, 0.3); // straighten
                if (group.Children[1] is ScaleTransform scale)
                {
                    AnimateTransform(scale, "ScaleX", 1.3, 0.3);
                    AnimateTransform(scale, "ScaleY", 1.3, 0.3);
                }
            }
        }
    }

    private void ScatteredAlbum_PointerExited(object sender, PointerRoutedEventArgs e)
    {
        if (sender is Grid grid && grid.DataContext is ScatteredAlbumModel model)
        {
            var container = VisualTreeHelper.GetParent(grid) as ContentPresenter;
            ...
            {
                if (group.Children[0] is RotateTransform rotate)
                    AnimateTransform(rotate, "Angle", model.Rotation, 0.3); // restore
                if (group.Children[1] is ScaleTransform scale)
                {
                    AnimateTransform(scale, "ScaleX", 1.0, 0.3);
                    AnimateTransform(scale, "ScaleY", 1.0, 0.3);
                }
            }
        }
    }

    private void ScatteredAlbum_PointerPressed(object sender, PointerRoutedEventArgs e)
    {
        if (sender is Grid grid && grid.DataContext is ScatteredAlbumModel model)
        {
            var album = model.Album.Title;
            var playlist = new SongsTabInfo
            {
                Name = album,
                Icon = "\uE93C",
            ...
            ViewModel.SelectAlbumCommand.Execute(model.Album);
        }
    }
