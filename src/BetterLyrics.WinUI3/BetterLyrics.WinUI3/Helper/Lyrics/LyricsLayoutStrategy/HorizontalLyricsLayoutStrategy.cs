    ...
    public class HorizontalLyricsLayoutStrategy : LyricsLayoutStrategyBase
    {
                    ...
                    double lyricsHeight)
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    double relativeX = alignment switch { TextAlignmentType.Center => (actualWidth - line.TertiaryTextLayout.LayoutBounds.Width) / 2, TextAlignmentType.Right => actualWidth - line.TertiaryTextLayout.LayoutBounds.Width, _ => 0 };
                    line.TertiaryPosition = line.TertiaryPosition.AddX((float)(offsetX + relativeX));
                }

                if (line.PrimaryTextLayout != null)
                {
                    double relativeX = alignment switch { TextAlignmentType.Center => (actualWidth - line.PrimaryTextLayout.LayoutBounds.Width) / 2, TextAlignmentType.Right => actualWidth - line.PrimaryTextLayout.LayoutBounds.Width, _ => 0 };
                    line.PrimaryPosition = line.PrimaryPosition.AddX((float)(offsetX + relativeX));
                }

                if (line.SecondaryTextLayout != null)
                {
                    double relativeX = alignment switch { TextAlignmentType.Center => (actualWidth - line.SecondaryTextLayout.LayoutBounds.Width) / 2, TextAlignmentType.Right => actualWidth - line.SecondaryTextLayout.LayoutBounds.Width, _ => 0 };
        ...
        )
        {
            ...
            int end = FindLastVisibleLine(lines, offset, lyricsY, lyricsHeight);
        ...
        private static int FindLastVisibleLine(IList<RenderLyricsLine> lines, double offset, double lyricsY, double lyricsHeight)
        {
            int left = 0, right = lines.Count - 1, result = -1;
            while (left <= right)
            {
