...
namespace BetterLyrics.Core.Helpers
{
    public class LatestOnlyTaskRunner
    {
        ...
        public async Task<T?> RunAsync<T>(Func<CancellationToken, Task<T>> taskFactory, int maxRetries = 1, int delayMilliseconds = 1000)
        {
            _cts?.Cancel();
            _cts?.Dispose();
            ...
            for (int attempt = 1; attempt <= maxRetries; attempt++)
            {
                try
                {
                    Debug.WriteLine($"RunAsync<{typeof(T).Name}>: Starting task {taskName} (Attempt {attempt}/{maxRetries}) with token {tokenHashCode}.");

                    T result = await taskFactory(token);

                    Debug.WriteLine($"RunAsync<{typeof(T).Name}>: Task {taskName} completed successfully on attempt {attempt}.");
                    return result;
                }
                catch (OperationCanceledException)
                {
                    Debug.WriteLine($"RunAsync<{typeof(T).Name}>: Task {taskName} with token hash code {tokenHashCode} was cancelled. Stopping retries.");
                    return default;
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"RunAsync<{typeof(T).Name}>: Task {taskName} threw an exception on attempt {attempt}: {ex.Message}");

                    if (attempt == maxRetries)
                    {
                        Debug.WriteLine($"RunAsync<{typeof(T).Name}>: Task {taskName} failed after {maxRetries} attempts. Giving up.");
                        return default;
                    }
