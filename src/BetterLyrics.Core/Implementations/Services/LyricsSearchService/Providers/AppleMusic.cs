...
namespace BetterLyrics.Core.Implementations.Services.LyricsSearchService.Providers
{
    ...
    {
        private readonly IPlatformProvider _platformProvider = Ioc.Default.GetRequiredService<IPlatformProvider>();
        ...
        public async Task<bool> InitAsync(CancellationToken cancellationToken)
        {
            if (!_isInited)
            {
                var mediaUserToken = _platformProvider.GetCredential(Core.Constants.App.AppName,
                    Core.Constants.AppleMusic.MediaUserTokenKey);
        ...
        private async Task SetMediaUserTokenAsync(string token, CancellationToken cancellationToken)
        {
            ...
            var resp = await _client.GetStringAsync("https://amp-api.music.apple.com/v1/me/storefront",
                cancellationToken);
            var json = JsonSerializer.Deserialize(resp, Core.Serialization.SourceGenerationContext.Default.JsonElement);
