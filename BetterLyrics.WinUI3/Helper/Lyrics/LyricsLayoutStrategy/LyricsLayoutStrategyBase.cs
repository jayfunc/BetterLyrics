...
namespace BetterLyrics.WinUI3.Helper.Lyrics.LyricsLayoutStrategy
{
    ...
    {
        public abstract void MeasureAndArrange(ICanvasAnimatedControl resourceCreator, IList<RenderLyricsLine>? lines, LyricsWindowStatus status, AppSettings appSettings, double canvasWidth, double canvasHeight, double lyricsWidth, double lyricsHeight);
        public abstract double? CalculateTargetScrollOffset(IList<RenderLyricsLine>? lines, int playingLineIndex);
        public abstract (int Start, int End) CalculateVisibleRange(IList<RenderLyricsLine>? lines, double currentScrollOffset, double lyricsOffset, double lyricsSize, double playingLineOffsetFactor);
        public abstract int FindMouseHoverLineIndex(IList<RenderLyricsLine>? lines, bool isMouseInLyricsArea, Point mousePosition, double currentScrollOffset, double lyricsOffset, double lyricsSize, double playingLineOffsetFactor);
