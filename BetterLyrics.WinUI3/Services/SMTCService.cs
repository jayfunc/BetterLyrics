            ...
            if (!string.IsNullOrEmpty(track.LocalAlbumArtPath))
            {
                _ = Task.Run(async () =>
                {
                    if (File.Exists(track.LocalAlbumArtPath))
                    {
                        ...
                        AppUIThread.Execute(() => { item.ApplyDisplayProperties(props); });
                    }
                });
            }
        ...
        private async void Binder_Binding(MediaBinder sender, MediaBindingEventArgs args)
        {
            ...
            try
            {
                ...
                args.SetStream(sourceStream.AsRandomAccessStream(),
                    MimeMapping.MimeUtility.GetMimeMapping(track.FileName));
            }
            catch (Exception ex)
            {
                AppUIThread.Execute(() => GlobalToastManager.Show("Error", ex.Message, MessageSeverity.Error));
            }
            finally
            {
                deferral.Complete();
