...
namespace BetterLyrics.WinUI3.Controls
{
    public sealed partial class InAppNotificationStack : UserControl
    {
        ...
        public void Show(string title, string? message, InfoBarSeverity severity = InfoBarSeverity.Informational, TimeSpan? duration = null, bool isClosable = true)
        {
            ...
            {
                _ = HandleAutoCloseAsync(item, duration.Value);
            }
        }
        ...
        private async void RemoveItem(NotificationItem item)
        {
            ...
            if (_itemContainerMap.TryGetValue(item, out var container))
            {
                await PlayExitAnimationAsync(container);
            }
        ...
        {
            if (sender.DataContext is NotificationItem item)
            {
                RemoveItem(item);
            }
        }
            ...
            for (int i = activeItems.Count - 1; i >= 0; i--)
            {
                ...
                if (_itemContainerMap.TryGetValue(item, out var element))
                {
                    ...
                    if (translateTransform != null && scaleTransform != null)
                    {
                        ...
                        double itemHeight = element.ActualHeight > 0 ? element.ActualHeight : 68;
                        ...
                        {
                            int stackIndex = (activeItems.Count - 1) - i;
                            int visibleStackLimit = 3;
                            int visualIndex = Math.Min(stackIndex, visibleStackLimit);
        ...
        private void AnimateTo(TranslateTransform trans, ScaleTransform scale, FrameworkElement element, double targetY, double targetZ, double targetScale)
        {
            var sb = new Storyboard();
            ...
            receiver.Translation = new System.Numerics.Vector3(currentTrans.X, currentTrans.Y, (float)targetZ);
        ...
        private void OnItemContainerLoaded(object sender, RoutedEventArgs e)
        {
            if (sender is Grid rootGrid && rootGrid.DataContext is NotificationItem item)
            {
                ...
                var receiver = rootGrid.Children[1] as UIElement;
